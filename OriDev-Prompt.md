...
- **SSH Terminal**: Vollwertiges Terminal-Emulator mit Tastatur-Shortcuts. Layout-Verhalten: Automatische Bildschirmdrehung aktiviert. Im Landscape-Modus: Dual-Panel-Split – oberes Panel zeigt den Terminal-Output (scrollbar, selektierbar), unteres Panel die Tastatur. Im Portrait-Modus: Terminal fullscreen, Tastatur als Bottom-Sheet. Drei wählbare Tastatur-Modi (Settings → Terminal → Tastatur-Stil, Phase 14):
    - **Built-in keyboard (CUSTOM, default)**: in-app Soft-Keyboard mit Terminal-Sondertasten (Tab, Ctrl, Alt, Esc, Arrow Keys, Fn-Reihe, Pipe/Slash/Tilde). Privacy-preserving — nichts wird an die System-IME weitergeleitet, daher kein Dictionary-Learning oder Cloud-Sync von Passwörtern. Empfohlen für Passwort-Eingaben. Split-Ratio per Drag anpassbar (Default 50/50, Range 0.2–0.8).
    - **System keyboard with shortcut row (HYBRID)**: Android System-IME (Gboard, SwiftKey etc.) für normalen Text, plus sticky 52dp-Toolbar mit den Terminal-Sondertasten direkt über der IME. KeyboardType.Password + autoCorrect=false auf dem unsichtbaren IME-Anchor unterdrückt Dictionary-Learning bei den großen IMEs. Beim Umschalten Warn-Dialog mit IME-Privacy-Hinweis.
    - **System keyboard only (SYSTEM_ONLY)**: nur die System-IME, ohne Shortcut-Row. Power-User-Escape-Hatch.
